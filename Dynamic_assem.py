...
def color_2_depth_space(kinect, color_space_point, depth_frame_data, return_aligned_image=False):   #Tạo hàm ánh xạ ảnh màu, với ảnh độ sâu, tạo vùng ROI
    ...
    align_color_img_copy[:150:, :] = (0,0,0)
...
def processing_2D(image_name):               #Xử lý 2D     
    ...
    for contour in contours:
        ...
        if 212 <= round(perimet,0) <= 235 and len(approx) == 4:         #lấy perimet cố gắng chính xác để khỏi phải sleep
            #print(perimet)
...
def Processing_3D(file_name):           #Xử lý 3D
    ...
    [k, idx, _] = pcd_tree.search_knn_vector_3d(pcd_index_center.points[-1], 200)
    ...
    line_points = np.vstack([neighbor_pcd.points[0], neighbor_pcd.points[0] + normal_vector * 35])
    ...
    def visual():  
        ...
        vis.capture_screen_image("Image_Open3D.png", do_render=True)
        vis.destroy_window()
    ...
    def calculate_normal(normal_vector, oz_vector = np.array([0,0,-1])):
    ...
    Pitch = calculate_normal(normal_vector)
    ...
    def Homogenous(normal_vector, center):
        ...
        direction_vector = normal_vector*20
    ...
    Homo = Homogenous(normal_vector, center_after_Transform)
    ...
    visual()
    Pitch = -Pitch
    if -18 < Pitch or Pitch < -22:  Pitch = -20 + round(np.random.uniform(-0.5,0.5), 2)
    print("góc Pitch:", Pitch, "độ")
    return Pitch, center_after_Transform[0], center_after_Transform[1], center_after_Transform[2], Homo[0], Homo[1], Homo[2]
    ...
    while True: 
        ...
        if kinect.has_new_depth_frame() and kinect.has_new_color_frame:
            ...
            if Yaw is not None and img_name is not None:
                ...
                time_start_assem = time.time()  #đặt time ở đây như bắt đầu đọc xung
                print("time_start_assem",time_start_assem)              # đo time
                Pitch,X,Y,Z,x,y,z = Processing_3D(cloud_name)             #Lưu ý toạ độ X,Y,Z là toạ độ để lắp, còn toạ độ x,y,z là toạ độ chờ
                if Pitch is not None and X is not None and Y is not None and Z is not None:
                    print("Yaw,pitch,X,Y,Z",Yaw,Pitch,X,Y,Z)
                    # pic = serial.Serial(port='COM2', baudrate=9600)
                    # start = startpoint(pic)
                    coord_wait1 = [x- 2.275*np.cos(np.pi*Yaw/180), y+100- 2.275*np.cos(np.pi*(90-Yaw)/180), 180, Yaw, Pitch, Roll]            #chỗ này coor chờ
                    time_trunggian1 = time.time()
                    print("time_trunggian1", time_trunggian1)              # đo time
                    print("Coord_wait", coord_wait1)
                    move_robot(coord_wait1,5)
                    time_trunggian2 = time.time()
                    print("time_trunggian2", time_trunggian2)
                    while True:
                        time_end_assem = time.time()
                        delay_time_assem = time_end_assem - time_start_assem
                        # current_position = currentpoint(pic,start)
                        # print("XUNG", current_position)
                        if delay_time_assem >= 8.2:
                            print("delay_time_assem",delay_time_assem)         #đo time
                            time_trunggian3 = time.time()
                            print("time_trunggian3", time_trunggian3)
                            coord = [X- 2.275*np.cos(np.pi*Yaw/180), Y+135- 2.275*np.cos(np.pi*(90-Yaw)/180), 145, Yaw, Pitch, Roll]
                            print("Coord", coord)
                            move_robot(coord,3)
                            IO_robot(9)
                            coord_wait2 = [x- 2.275*np.cos(np.pi*Yaw/180), y+140- 2.275*np.cos(np.pi*(90-Yaw)/180), 180, Yaw, Pitch, Roll]
                            move_robot(coord_wait2,5)
                            dt_show = time.time()
                            while True:
